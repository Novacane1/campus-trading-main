...
from app.models.models import Item, UserAction, User, Category, Order, ItemEmbedding
...
class RecommendationService:
    ...
    def get_recommendations(user_id, limit=FINAL_LIMIT, refresh=False):
        ...
        # 校园隔离：获取用户学校
        school = _get_user_school(user_id)
        school_ids = _school_item_ids_filter(school)

        # 2. 多路召回
        cf_ids = RecallEngine.collaborative_filtering_recall(user_id)
        content_ids = RecallEngine.content_based_recall(user_id)
        cluster_ids = RecallEngine.cluster_recall(user_id)
        hot_ids = RecallEngine.hot_recall(school_name=school)
        ...
        # 3. 去重合并（保持优先级：CF > 内容 > 聚类 > 热度）
        seen = set()
        merged = []
        for id_list in [cf_ids, content_ids, cluster_ids, hot_ids]:
            for iid in id_list:
                if iid not in seen:
                    # 校园隔离：跳过非同校商品
    ...
    def get_similar_items(item_id, limit=10, user_id=None):
        """
        商品详情页"猜你喜欢" - TF-IDF内容相似 + Item-CF + 同类别补充
        """
            ...
            if item_id in item_id_to_idx:
                ...
                for i, score in enumerate(sims):
                    ...
                    if iid != item_id:
                        if school_ids is not None and iid not in school_ids:
                            continue
                        candidate_scores[iid] += float(score) * 2.0  # 内容相似权重

        # 2. Item-CF: 看过这个商品的人还看了什么
    ...
    def get_hot_items(limit=10, user_id=None):
        """热门商品推荐（校园隔离）"""
        school = _get_user_school(user_id)
        cache_key = f'rec:hot:{limit}:{school or "all"}'
        try:
            if redis_client:
                cached = redis_client.get(cache_key)
                if cached:
        ...
        hot_ids = RecallEngine.hot_recall(limit, school_name=school)
